...
public class Product {
	...
	public List<Product> filterProducts(Double price){
		//fill your code here
		List<Product> filterList=getAllProducts();
		List<Product> finalList=filterList.stream().filter(f -> f.getPrice()<=price).collect(Collectors.toList());
		return finalList;
	}

	public String toString(){
		return String.format("%-20s %-20s %-20s %-10s %s\n",getName(),getBrand(),getProductCategory(),getColor(),Double.valueOf(getPrice()));
}
